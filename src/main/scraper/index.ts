...
import { axiosMidlandResidentialData, axiosMilandCommercialData } from "./axiosRequests"
...
export const requestMilandCommercialData = async ({
  ...
  originalEndDate,
    ...
    const { finalResult, finalComb, totalNumber, passNumber } = handleCommercialCombs({
        usagePriceDict,
        usage,
        t,
        combNumber,
...
export const requestMilandResidentialData = async ({
    ...
    while ((!finalResult || !finalComb) && maxTrials >= 0) {
      ...
      const { tempResult: r, tempComb: c, totalNumber: tt, passNumber: p} = findResultAndCombo({
        resResult,
        combNumber,
        usagePriceDict,
        startDate,
