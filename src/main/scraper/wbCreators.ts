    ...
    if (modeSelection === "single") {
      ...
      for (const [regionId, regionList] of Object.entries(chosenTable)) {
        ...
        for (const reg of regionList) {
          const regSplit = reg.split("/");
          for (const r of regSplit) {
            if (a === r.trim()) {
...
export const constructWb = async ({
  ...
  if (!!finalResult && !!finalComb) {
    targetSheet.getCell(`${resultColDict[usage][t]}${rowIndex}`).value =
      finalResult;
    ...
    resultWs.getCell(`K${writeRow + 1}`).value = `=${
      passNumber === 0 || totalNumber === 0
        ? 0
        : (100 * passNumber) / totalNumber
    }%`;
    ...
    mainWindow.webContents.send(
      "error:push",
      `Could not find row ${rowIndex} result (${usage}-${t}) within 10% difference of valuation price!`,
    );
  }
};
