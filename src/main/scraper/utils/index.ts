...
import {PriceRangeDict, txType, usageColDict, UsagePriceDict, UsageTypes} from '../constants'
...
export const setUpResidentialToken = async (): Promise<string| undefined >=> {
 ...
 export const getUsageList = ({ ws, rowIndex, mainWindow }: getUsageListArgs): UsageTypes[] => {
  ...
  export const makeUsagePriceDict = ({ ws, rowIndex, usages, mainWindow }:makeUsagePriceDictArgs): UsagePriceDict => {
        ...
        if (t === "rental") {
          ...
          if (!!rentalValuation && !!rentalActual) {
            ...
            if (
              ...
              actualRange[1] <= valuationRange[1] && valuationRange[1] <= actualRange[0]
            ) {
